...
> **Rule:** Consumers import from `@b3/ui`. UI implementation code imports Panda helpers from `@b3/styled-system/*`.
...
`@b3/preset` exports `b3Preset`, re-exported from `src/preset.ts` for compatibility, assembled from:
...
### `@b3/styled-system` is the shared Panda runtime

- `@b3/styled-system` is the generated Panda runtime package shared by `@b3/ui` and consuming apps
- Imports like `@b3/styled-system/jsx`, `@b3/styled-system/css`, and `@b3/styled-system/recipes` resolve through that workspace package
- `@pandacss/dev` is the **build-time tool** — used in `@b3/preset`, `@b3/styled-system`, and app-level Panda configs
- `@b3/ui` treats `@b3/styled-system` as an external runtime dependency during dist builds
...
| `src/preset.ts`                        | Compatibility re-export of `b3Preset` |
...
Apps configure `panda.config.ts` with `presets: ["@pandacss/preset-base", b3Preset]` and `importMap: "@b3/styled-system"`.
Path alias `@b3/ui` → `packages/ui/src` resolved by Vite/tsconfig.

Published consumers that do not scan `@b3/ui` source can include `@b3/ui/panda.buildinfo.json`
...
- **Aliases:** `@b3/styled-system` → `../styled-system/dist` in `vitest.config.ts`
